...
|---|---|---|
| `admin` | hub operator (you only) | the admin REST API (`/admin/v1/*`); CLI side: `hub admin invite mint` / `admin invite revoke\|restore\|extend` / `admin offering revoke\|restore` / `list` / `status` |
...
|---|---|---|---|
| `maxConcurrencyPerUser` | 1 | concurrent requests **per consumer** on this alias | 429 `PRODUCER_MAX_CONCURRENCY` — only after `queueWaitMs` of queueing (see below) |
...
| `AWESHARE_QUEUE_WAIT_MS` | 10000 | how long a request may wait for a per-user admission slot before 429 — short same-key overlaps (a coding agent's background title request, a second window) queue through instead of failing at once; `0` = reject immediately |
